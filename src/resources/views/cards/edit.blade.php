@extends('layouts.app')
@section('title', $card->full_name . ' - 編集')

@section('content')
<div class="space-y-6">
    <!-- ページヘッダー -->
    <div class="flex items-center gap-4">
        <a href="{{ route('cards.show', $card) }}" class="text-slate-400 hover:text-emerald-600 transition">
            <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 24 24"><path d="M19 12H5m7-7l-7 7 7 7"/></svg>
        </a>
        <div>
            <h2 class="text-2xl font-bold text-slate-900">名刺を編集</h2>
            <p class="text-sm text-slate-500 mt-0.5">{{ $card->full_name }}</p>
        </div>
    </div>
    ...
    <form action="{{ route('cards.update', $card) }}" method="POST" class="space-y-6">
            ...
            <div class="p-6 space-y-4">
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="last_name" value="{{ $card->last_name }}" required
                    ...
                    <div>
                        ...
                        <input type="text" name="first_name" value="{{ $card->first_name }}" required
                    ...
                    <div>
                        ...
                        <select name="company_id" required
                            ...
                            @foreach(\App\Models\Company::orderBy('name')->get() as $company)
                                <option value="{{ $company->id }}" {{ $card->company_id === $company->id ? 'selected' : '' }}>
                                    {{ $company->name }}
                                </option>
                            @endforeach
                        </select>
            ...
            <div class="p-6 space-y-4">
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="department" value="{{ $card->department }}"
            ...
            <div class="p-6 space-y-4">
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="tel" name="mobile" value="{{ $card->mobile }}"
                ...
                <div>
                    ...
                    <input type="tel" name="fax" value="{{ $card->fax }}"
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-100">
            ...
            <div class="p-6">
                <textarea name="memo" rows="5"
                          class="w-full px-4 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent @error('memo') border-red-500 @enderror">{{ $card->memo }}</textarea>
        ...
        <div class="flex items-center gap-3">
            ...
            <a href="{{ route('cards.show', $card) }}" class="flex items-center gap-2 px-6 py-2 border border-gray-200 rounded-lg font-semibold text-slate-700 hover:bg-slate-50 transition">
                キャンセル
            </a>
        </div>
    </form>
